  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (editing) {
        ...
        await fetch("/api/seasoning-ingredients", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) });
        toast.success("创建成功");
      }
      setDialogOpen(false);
      fetchData();
  ...
  return (
    <div className="flex flex-col gap-6 p-8">
      <div className="flex items-center justify-between">
        <PageHeader title="调料清单" description="标准化产品形态的基础调味品" />
        <Button onClick={openCreate}><Plus className="mr-2 h-4 w-4" />新增调料</Button>
      </div>
              ...
              <TableBody>
                {filtered.length === 0 ? (
                  ...
                  <TableRow key={row.id} className="transition-colors hover:bg-muted/40">
                    ...
                    <TableCell className="text-muted-foreground">{row.productSpec || "—"}</TableCell>
      ...
      <Dialog open={deleteId !== null} onOpenChange={() => setDeleteId(null)}>
        <DialogContent className="sm:max-w-[400px] [&>button]:cursor-pointer">
          ...
          <p className="text-muted-foreground text-base">确定要删除这条调料吗？此操作不可撤销。</p>
          <DialogFooter>
